...
|---|---|---|---|
| `{env}-ecs-cluster-main-public` | internet-facing | HTTPS:443 | Browser traffic → app-front containers |
| `{env}-ecs-cluster-main-private` | internal | HTTPS:443 + HTTP:80 | Service-to-service + Node.js internal calls |
...
| Resource | Value |
|---|---|
| Cluster name | `p-ecs-cluster-main` (prod) / `h-ecs-cluster-main` (homolog) |
...
|---|---|---|
| `environments/prod/` | `p-ecs-cluster-main` | `cluster-terraform/prod/kriolu-kloud-cluster-us-east-1.tfstate` |
| `environments/homolog/` | `h-ecs-cluster-main` | `cluster-terraform/homolog/kriolu-kloud-cluster-us-east-1.tfstate` |

Both environments share the same VPC (created by `network/`). The `base_name` local is `{env_first_char}-ecs-cluster-main` — e.g., `p-` for prod, `h-` for homolog.
...
```
cluster/
├── ecs_cluster_main/         ← reusable module
